...
import type { AvailabilityPayload, ContactPayload } from "./validation";
...
export function buildContactEmailHtml(data: ContactPayload) {
    ...
    <tr>
      <td align="center">
        <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:18px;border:1px solid #E6DDD8;padding:28px 32px;">
          <tr>
            <td style="font-size:11px;letter-spacing:0.18em;text-transform:uppercase;color:#6B6B6B;padding-bottom:12px;">
              New contact inquiry · Maa's Production
            </td>
          </tr>
          <tr>
            <td style="font-size:22px;line-height:1.3;font-weight:500;padding-bottom:18px;">
          ...
          <tr>
            <td>
              <table width="100%" cellpadding="0" cellspacing="0" style="font-size:14px;line-height:1.6;color:#2E2E2E;">
                <tr>
                  <td width="140" style="color:#6B6B6B;padding:4px 0;">Name</td>
                  <td style="padding:4px 0;"><strong>${fullName}</strong></td>
                </tr>
                <tr>
                  <td width="140" style="color:#6B6B6B;padding:4px 0;">Email</td>
                  <td style="padding:4px 0;">${email}</td>
                </tr>
                <tr>
                  <td width="140" style="color:#6B6B6B;padding:4px 0;">Subject</td>
          ...
          <tr>
            <td style="padding-top:18px;padding-bottom:8px;font-size:13px;font-weight:500;color:#2E2E2E;">
              Message:
            </td>
          </tr>
          <tr>
            <td style="font-size:14px;line-height:1.7;color:#2E2E2E;padding:12px;background:#FFF7F2;border-radius:8px;border:1px solid #E6DDD8;">
              ${message.replace(/\n/g, "<br />")}
            </td>
          </tr>
          <tr>
...
export async function sendContactEmail(data: ContactPayload) {
  ...
  await resend.emails.send({
    from: defaultFrom,
    to,
    subject: `Contact Form: ${data.subject} - Maa's Production`,
    replyTo: data.email,
    html,
  });
}
